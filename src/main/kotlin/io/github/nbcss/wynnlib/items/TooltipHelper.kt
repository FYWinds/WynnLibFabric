...
fun addRequirements(item: Equipment, tooltip: MutableList<Text>) {
    ...
    val levelText = Text.literal(
        ": " + if (level.isConstant()) level.lower().toString()
        else level.lower().toString() + "-" + level.upper().toString()
...
fun addRolledIdentifications(
    item: RolledEquipment,
    tooltip: MutableList<Text>
): Float? {
    ...
    Identification.getAll().forEach {
        ...
        if (value != 0) {
            ...
            if (stars > 0) {
                text.append(
                    Text.literal(MutableList(stars) { "*" }.reduce { x, y -> x + y })
